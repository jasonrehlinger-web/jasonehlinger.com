...
- **Never use em-dashes (U+2014) or en-dashes (U+2013) as sentence punctuation. Use commas, periods, colons, or parentheses instead.** This applies to the post body, frontmatter (title, description), and CTA copy. En-dashes in number ranges (e.g. 800–1,300) are fine.
...
- **Publish method (primary): GitHub web UI via Claude in Chrome** (Jason is logged into GitHub in Chrome, no tokens needed):
  ...
  5. **Verify before publishing:** re-open the committed file on GitHub in raw view (`https://github.com/jasonrehlinger-web/jasonehlinger.com/raw/main/src/content/blog/<slug>.md`) and confirm (1) the full file is present and ends with the final closing tag/line you wrote, (2) there are no null bytes or garbage characters, and (3) it contains zero em-dashes (U+2014). If any check fails, re-upload the correct full content. The site build runs an integrity check (`scripts/check-integrity.mjs`) that blocks deploys of truncated files, so a failed check means the post will not go live.
  6. Wait ~60–90s, then load `https://jasonehlinger.com/blog/<slug>/` and confirm it renders (category tag, CTAs, no stray markdown symbols).
...
- Zero em-dashes (U+2014) anywhere in the file, and no en-dashes used as sentence punctuation.
